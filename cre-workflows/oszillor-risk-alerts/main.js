...
var onHttpTrigger = (runtime2) => {
  const config = runtime2.config;
  const triggerData = runtime2.triggerEvent;
  const bodyStr = new TextDecoder().decode(triggerData.input);
  const alert = JSON.parse(bodyStr);
  runtime2.log(`Alert subscription: ${alert.subscriber} condition=${alert.condition} threshold=${alert.threshold}`);
  const reportData = encodeAbiParameters(parseAbiParameters("address subscriber, uint8 condition, uint256 threshold, string webhookUrl, uint256 ttl"), [
    alert.subscriber,
    alert.condition,
  ...
  const evmClient = new ClientCapability(baseNetwork.chainSelector.selector);
  ...
  runtime2.log(`Alert written to AlertRegistry: ${txHash}`);
  return { ruleId: "pending", txHash };
};
var fetchRiskScore = (nodeRuntime) => {
  const evmClient = new ClientCapability(getNetwork({
    chainFamily: "evm",
  ...
  const result = evmClient.read(nodeRuntime, {
    contractAddress: nodeRuntime.config.vaultEvm.vaultAddress,
    abi: OszillorVault,
    functionName: "currentRiskScore"
  }).result();
  return result;
};
var fetchEmergencyMode = (nodeRuntime) => {
  const evmClient = new ClientCapability(getNetwork({
    chainFamily: "evm",
    chainSelectorName: nodeRuntime.config.vaultEvm.chainName,
    isTestnet: nodeRuntime.config.vaultEvm.chainName.includes("sepolia")
...
var fetchActiveRules = (nodeRuntime) => {
  const baseNetwork = getNetwork({
    chainFamily: "evm",
    chainSelectorName: nodeRuntime.config.alertEvm.chainName,
    isTestnet: nodeRuntime.config.alertEvm.chainName.includes("sepolia")
  });
  if (!baseNetwork)
    return [];
  const evmClient = new ClientCapability(baseNetwork.chainSelector.selector);
  const count = evmClient.read(nodeRuntime, {
    contractAddress: nodeRuntime.config.alertEvm.alertRegistryAddress,
    abi: AlertRegistry,
  ...
  if (count === 0n)
    return [];
  const rules = [];
  const maxRules = count > 50n ? 50n : count;
  for (let i2 = 0n;i2 < maxRules; i2++) {
    const isActive = evmClient.read(nodeRuntime, {
      contractAddress: nodeRuntime.config.alertEvm.alertRegistryAddress,
      abi: AlertRegistry,
    ...
    if (!isActive)
      continue;
    const rule = evmClient.read(nodeRuntime, {
      contractAddress: nodeRuntime.config.alertEvm.alertRegistryAddress,
      abi: AlertRegistry,
...
var onCronTrigger = (runtime2) => {
  const riskScore = runtime2.runInNodeMode(fetchRiskScore, consensusMedianAggregation())().result();
  const emergencyMode = runtime2.runInNodeMode(fetchEmergencyMode, consensusMedianAggregation())().result();
  const isEmergency = emergencyMode === 1n;
  runtime2.log(`Current risk score: ${riskScore}, emergency: ${isEmergency}`);
  let ruleLines = [];
  try {
    ruleLines = runtime2.runInNodeMode(fetchActiveRules, consensusMedianAggregation())().result();
  } catch {
    runtime2.log("Failed to fetch active rules from AlertRegistry");
    return { evaluated: 0, triggered: 0 };
  }
  if (ruleLines.length === 0) {
    runtime2.log("No active alert rules to evaluate");
    return { evaluated: 0, triggered: 0 };
  }
  let triggered = 0;
  for (const line of ruleLines) {
    ...
    if (parts.length < 5)
      continue;
    const [ruleId, subscriber, conditionStr, thresholdStr, webhookUrl] = parts;
    const condition = Number(conditionStr);
    const threshold = BigInt(thresholdStr ?? "0");
    let shouldTrigger = false;
    switch (condition) {
      case 0:
        shouldTrigger = riskScore > threshold;
        break;
      case 1:
        shouldTrigger = riskScore < threshold;
        break;
      case 2:
        shouldTrigger = isEmergency;
        break;
    }
    if (shouldTrigger) {
      triggered++;
      runtime2.log(`ALERT TRIGGERED: rule=${ruleId} subscriber=${subscriber} ` + `condition=${condition} threshold=${threshold} risk=${riskScore} ` + `webhook=${webhookUrl}`);
    }
  }
  runtime2.log(`Evaluated ${ruleLines.length} rules, ${triggered} triggered`);
  return { evaluated: ruleLines.length, triggered };
};
